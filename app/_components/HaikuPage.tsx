...
function HaikuPoem({
  ...
  const handleClickHaiku = () => {
    const haikuToCopy = poem.map((s: string, i: number) => {
      ...
      .join("\n")
      + `\n—"${capitalize(haiku.theme)}", generated by ${isHaikudleMode ? "haikudle.art" : "haiku.desmat.ca"} (${moment(haiku.createdAt).format("YYYY")})\n`;
  ...
  if (solved || !isPuzzleMode) {
    return (
      <div
        className="_bg-purple-200 flex flex-col cursor-copy"
      ...
      >
        {poem.map((s: string, i: number) => (
          <StyledLayers key={i} styles={styles}>
            <div className="m-[0.2rem] transition-all">
