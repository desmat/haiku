...
const formatHaikuTitleAndAuthor = (haiku: Haiku, mode?: string) => {
  ...
  if (title.toLowerCase().startsWith("by") ||
    title.toLowerCase().startsWith("poetry by") ||
    title.toLowerCase().startsWith("art by") ||
    title.toLowerCase().startsWith("art and") ||
    title.startsWith("-") ||
    title.startsWith("—") ||
    title.startsWith("—") ||
    title.startsWith('"')) {

    const credits = title.split(" by ");
    if (credits.length >= 2) {
      credits[1] = ` by ${credits[1]}`;
    }
            ...
            >
              ...
              >
                ...
                {!showcaseMode && (copyAllowed || editAllowed || regeneratePoemAllowed) &&
                  <div
                    className={`onboarding-container group/actions _bg-yellow-200 flex flex-row gap-2 mt-auto md:pt-[0rem] sm:pt-[0.0rem] md:pb-[0.2rem] sm:pb-[0.5rem] pb-[0.4rem] md:pl-[0.9rem] sm:pl-[0.7rem] pl-[0.5rem] ${editing ? "" : "opacity-60"} group-hover:opacity-100 transition-opacity`}
                  >
