...
export async function _generateLimerick(user: any, lang?: LanguageType, startingWith?: string): Promise<Haiku> {
  console.log(">> services.haiku.generateLimerick", { lang, startingWith, user });
  const language = supportedLanguages[lang || "en"].name;
  const debugOpenai = process.env.OPENAI_API_KEY == "DEBUG";
  ...
  console.log(">> services.haiku.generateLimerick", { poem, title, poemPrompt });
  ...
  const imageRet = await fetch(imageUrl);
  // console.log(">> services.haiku.generateLimerick", { imageRet });

  const imageBuffer = Buffer.from(await imageRet.arrayBuffer());
  console.log(">> services.haiku.generateLimerick", { imageBuffer });

  const getColors = require('get-image-colors')

  const colors = await getColors(imageBuffer, 'image/png');
  console.log(">> services.haiku.generateLimerick", { colors });
  ...
  // console.log(">> services.haiku.generateLimerick", { subject, filename, blob });
