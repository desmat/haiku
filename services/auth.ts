...
import { Auth, User, signInWithPopup, GoogleAuthProvider, GithubAuthProvider, createUserWithEmailAndPassword, signOut, signInWithEmailAndPassword } from "firebase/auth";
...
export async function signin(method: SigninMethod, params?: any) {
  ...
  if (["google", "github"].includes(method)) {
    const providerClass = method == "google"
      ? GoogleAuthProvider
      : method == "github"
        ? GithubAuthProvider
        : undefined;

    if (!providerClass) throw `Unknown signin method: ${method}`;
